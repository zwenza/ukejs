export default () => {
    ...
    self.run = function() {
        ...
        self._rootElement.appendChild(self.createElement(activeRoute.component.render(activeRoute.component.props)));
    }
...
self.createElement = function(node) {
    if(typeof node === 'string') {
        ...
        var filteredComponenet = self._registeredComponents.filter(component => component.selector === node.type);
        if(!filteredComponenet || filteredComponenet.length === 0) {
            console.log('Cannot find component with selector: ', node.type);
        } else {
            return self.createElement(filteredComponenet[0].render(filteredComponenet[0].props));
        }
    } else {
